...
# --- NEW: Python selection via Windows 'py' launcher ---
# Examples:
#   "-3.10"     -> Python 3.10
#   "-3.11"     -> Python 3.11
#   "-3.10-64"  -> Python 3.10 64-bit (if installed/registered)
# Set to "" to use default 'py' behavior (not recommended).
PY_LAUNCHER_SELECTOR = "-3.11"
# ------------------------------------------------------
...
def build_sbom_parts_inside_venv(py: Path, requirements_path: Path) -> Dict[str, Any]:
    ...
    Returns only packages reachable from requirements roots, and enriches each component with:
    description/licenses/externalReferences derived from installed package metadata.
    """
    helper_code = r"""
import json, re, sys
...
def create_venv(venv_dir: Path) -> None:
    """
    Create a venv using Windows 'py' launcher (allows Python version selection),
    falling back to sys.executable on non-Windows platforms.
    """
    if os.name == "nt":
        selector = PY_LAUNCHER_SELECTOR.strip()
        cmd = ["py"]
        if selector:
            cmd.append(selector)
...
def main() -> int:
    ...
    try:
        # 1) Create brand-new venv (Windows: use 'py' to select version)
        create_venv(venv_dir)
        py = venv_python(venv_dir)

        # 2) Install requirements into the venv
        run([str(py), "-m", "pip", "install", "-r", str(Config.requirements_txt_file_path.resolve()), *PIP_EXTRA_ARGS])
        ...
        # Keep a root dependency node to anchor the graph (metadata.component is NOT in components[])
        dependencies.insert(0, {"ref": root_bom_ref, "dependsOn": sorted(set(root_refs))})

        # 5) Write final SBOM in the requested top-level format/order
        out: Dict[str, Any] = {}
